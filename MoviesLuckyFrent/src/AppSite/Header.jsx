            ...
            {authenticated ? (
                <li className="relative group">
                    <Link to={`/profile/${users?.id}`}
                        ...
                        <img src={profile} alt="icon" style={{ width: '35px', height: '35px', display: 'block', border: '2px solid white', padding:'5px' , borderRadius:"50%" }} />
                    </Link>

                    {/* Dropdown menu */}
                    <ul className="absolute left-0 mt-2 space-y-2 bg-none text-gray-200 font-semibold shadow-lg rounded-lg py-2 w-48 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50">
                        {/* <li className="border-white border-b-4">
                            <Link to="/setting" className="px-4 py-2 hover:text-blue-500 flex justify-evenly items-center">
                                Setting
                                <img src={setting}  className='w-7 h-7' alt="icon" />
                            </Link>
                        </li> */}
                        <li className="border-white border-b-4 px-4">
                            <button onClick={onSubmit} className=" px-4 py-2 hover:text-blue-500 flex justify-evenly items-center">
                                Logout
                ...
                <li>
                    {/* Sign In Button */}
                    <Link to="/login" className="relative inline-flex items-center justify-start inline-block px-[35px] py-[10px] overflow-hidden font-bold rounded-md group">
                    <span className="w-32 h-32 rotate-45 translate-x-12 -translate-y-2 absolute left-0 top-0 bg-white opacity-[3%]"></span>
                    <span className="absolute top-0 left-0 w-48 h-48 -mt-1 transition-all duration-500 ease-in-out rotate-45 -translate-x-56 -translate-y-24 bg-white opacity-100 group-hover:-translate-x-8"></span>
                    <span className="relative w-full text-left text-white transition-colors duration-200 ease-in-out group-hover:text-gray-900">Sign In</span>
      ...
      {isOpen && (
        <ul className="md:hidden flex flex-col items-center bg-black text-white space-y-4 mt-4 py-4">
          ...
          {authenticated ? (
            ...
            {/* <li className="flex items-center space-x-2">
            <Link to="/setting" className="hover:text-blue-500 px-1 py-2 text-white rounded flex items-center">
                Setting
            </Link>
            <img src={setting} className="w-7 h-7" alt="setting icon" />
            </li> */}
            <li className="flex items-center space-x-2">
